...
#---------------------------------------MATRIZ DOS-----------------------------------------------------
#CREACION DE NUMERO DE FILA Y COLUMNA DE MATRIZ DOS
...
for f2 in range(fila2): 
    for c2 in range(columna2):
        array2[f2][c2]=float(input("ELEMENTO A INGRESAR EN LA POSICION " + str(f2) + (",") + str(c2) + ": "))
...
#-----------------------------------------FUNCIONALIDAD---------------------------------------------
array3=[]             #DECLARACION DE ARRAY RESULTADO
...
#MULTIPLICACION
for f1 in range(fila1):
    for c1 in range(columna2):
        for c2 in range(columna2):
            #ERROR QUE TENIA EN ESTA LINEA: can't multiply sequence by non-int of type 'str'
            array3[f1][c2]+=array1[f1][c1]*array2[c1][c2]  #Se sumaran los resultados, (+=) es un acumulador
print("EL RESULTADO EN MATRIZ ES: ")
for e in array3:
    print(e)
#El error se resolvio, añadiendo float al momento de pedir el dato de entrada para el valor de la matriz